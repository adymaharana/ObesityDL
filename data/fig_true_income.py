...
for city in cities:
    ...
    for path in paths:
        ...
        try:
            ...
            pcc = truepcc[tractidx]
            pcc_norm = (pcc - pccmin) / (pccmax - pccmin)
            fcol = colors(pcc_norm)
            mapped_true += 1
        except (KeyError, ValueError) as e:
            fcol = "0.5"
        patch = mpatches.PathPatch(path, facecolor=fcol, edgecolor='white')
        ax.add_patch(patch)
        counter += 1
    print('Total number of census tracts mapped to true values: ', mapped_true)

    sm = plt.cm.ScalarMappable(cmap=cmapstring, norm=plt.Normalize(vmin=int(pccmin), vmax=int(pccmax)))
    sm._A = []
    cbar = plt.colorbar(sm, orientation='horizontal', fraction=0.040, pad=0.04)
    ticks = list(np.linspace(pccmin, pccmax, 10))
    # print(ticks)
    ticks = [int(num/1000) for num in ticks]
    # print(ticks)
    cbar.ax.set_xticklabels(ticks, fontsize=25, weight='bold')  # change fontsize to 25 with 300 dpi
    ...
    ax.axis('off')
    plt.savefig(os.path.join('../out', city, city + out_file_suffix + '.png'), bbox_inches='tight', dpi=300)
